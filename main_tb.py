...
class Win(tk.Tk):
    def __init__(self):
        ...
        self.geometry("%dx%d+%d+%d" % (200,40,(self.winfo_screenwidth()-200)/2,self.winfo_screenheight()-40))#设置窗口大小
        self.attributes("-transparentcolor",bg)#设置窗口背景透明
        self.attributes("-topmost",True)#设置窗口置顶
        self.top()
    def top(self):#此处会在每0.1秒后重新设置置顶，防止窗口被优先级更高的任务栏盖住
        #print('重新置顶')
        self.attributes("-topmost",True)#设置窗口置顶
        self.after(100,self.top)#设定0.1秒后再次执行自己
#界面
class Desktop(tk.Frame):
    def __init__(self,master):
        ...
        self.time_label=tk.Label(self,text=self.time_now,background=bg,foreground=fg)#显示时间的标签
        ...
        #展开框架
        self.more_frame=tk.Frame(self,bg=bg)
    ...
    def get_time(self):#获取时间
        ...
        self.master.after(100,self.get_time)#0.1秒后再次执行此函数，防止反应过来时一秒已经快结束了，实测有改善
    def move(self,event):#移动事件
        new_x=(event.x-self.x)+self.master.winfo_x()
        #new_y=(event.y-self.y)+self.master.winfo_y()
        if self.more==0:
            self.master.geometry("%dx%d+%d+%d"%(200,40,new_x,self.winfo_screenheight()-40))
        else:
            self.master.geometry("%dx%d+%d+%d"%(200,150,new_x,self.winfo_screenheight()-150))
    def reduce(self,event=None):#还原
        self.master.geometry("%dx%d+%d+%d"%(200,40,(self.winfo_screenwidth()-200)/2,50))
        try:
    ...
    def more_(self,event=None):#展开
        origin_y=self.master.winfo_y()
        if self.more==0:#没有展开
            self.more=1
            self.more_button.config(text="↓")
            self.master.attributes("-transparentcolor",'')#设置窗口背景不透明
            for i in range(0,(150-40)//10):#一个小动画细节:)
                self.master.geometry("%dx%d+%d+%d"%(200,150,self.master.winfo_x(),origin_y-(i)*10))
                self.master.update()
                time.sleep(0.01)
            self.master.geometry("%dx%d+%d+%d"%(200,150,self.master.winfo_x(),origin_y-(150-40)))
        else:#展开了
            self.more=0 
            self.more_button.config(text="↑")
            self.master.attributes("-transparentcolor",bg)#设置窗口背景透明
            for i in range(0,(150-40)//10):#一个小动画细节:)
                self.master.geometry("%dx%d+%d+%d"%(200,150,self.master.winfo_x(),origin_y+(i)*10))
                self.master.update()
                time.sleep(0.01)
            self.master.geometry("%dx%d+%d+%d"%(200,40,self.master.winfo_x(),origin_y+(150-40)))
    ...
    def start_memory(self,event=None):#打开内存管理界面
        ...
        def quit_memory(event=None):#退出
            frame_memory.destroy()
        #创建内存管理界面框架
        frame_memory=tk.Frame(self.more_frame,bg=bg)
